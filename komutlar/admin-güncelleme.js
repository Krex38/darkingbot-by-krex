...
exports.run = async (client, message, args) => {
  ...
  if(args[0] === 'ekle') {
    if(!args[1]) return message.reply('Güncelleme başlığını yazmalısın. Her parametre arasına | koyman gerekiyor. Örnek: k!güncelleme ekle Başlık | Açıklama');
    args = args.slice(1).join(' ').split(' | ');
    if(!args[1]) return message.reply('Güncelleme açıklamasını yazmalısın. Her parametre arasına | koyman gerekiyor. Örnek: k!güncelleme ekle Başlık | Açıklama');
